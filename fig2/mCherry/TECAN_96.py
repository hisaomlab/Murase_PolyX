...
FI_YLIM = (0, 28000)     # GFP曲線のylim
FI_YTICKS = [14000, 28000]
...
MFI_YLIM = [0, 28000]           # MFIのylim
NI_YLIM = [0, 30000]     # NIのylim
TITLE = "mCherry-PolyX (SC-LU)"      # 図のタイトル
#-----------------------------
...
def create_lineplot(growth_df, fi_df):
    ...
    for n, AA in zip(range(len(sample_name)), sample_name):
        ...
        plot_filled_line(ax2, AA, fi_data, time, color="#FF8AF1", label="_nolegend_", ylim=FI_YLIM, yticks=FI_YTICKS)
...
def create_mainplot(results):
   ...
   # MGR 平均値と標準偏差の棒グラフ
    ...
    # MFI 平均値と標準偏差のプロット
    ax2 = ax.twinx()
    ax2.scatter(df_mean.index, df_mean['MFI'], color='#FF8AF1', marker='_', label='Expression level', s=150, edgecolor='black', linewidth=2.5, zorder=2, alpha=0.9)
    ...
    for AA in sample_name:
        ...
        ax2.scatter(x_positions, y_values_MFI, color='#FF8AF1', s=10, linewidths=0.1, zorder=3, alpha=0.7)
    ...
    for i, (p_val, rect) in enumerate(zip(p_mfi_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.8, '*', ha='center', color='#FF8AF1', fontsize=10)
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 4500, '*', ha='center', color='k', fontsize=10)
