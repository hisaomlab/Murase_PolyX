...
#-----読み込むファイルの指定-----
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig2/mCherry/20250117_mCherry_PolyX.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig2/mCherry/Sample_Sheet_Poly10X.xlsx" #サンプルシートを指定
#sample_name = ['Δ','D', 'Q', 'N', 'S', 'E', 'R', 'H', 'P', 'K', 'A', 'V', 'T','G', 'L', 'C','F','I','Y','M','W','Φ'] #サンプル名前を指定
sample_name = ['Φ','Δ','A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N','P', 'Q', 'R','S','T','V','W','Y'] #サンプル名前を指定
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig2/mCherry"#保存先のパス
...
def fourier_transform(Growth_df, RFP_df):
    #~~~~パラメータ~~~~
    Start = 35 #周期検出の最初
    End = 192 #周期検出の最後
    Threshold = 0.1 #閾値を設定
    ...
    RFP_ave_df =  RFP_df.rolling(period,axis=1,center=True,min_periods=1).mean()

    create_lineplot(Growth_ave_df, RFP_ave_df)
    calculation(Growth_ave_df, RFP_ave_df)
...
def create_lineplot(growth_df, rfp_df):
    fin_time = 80  # [hour]
    ...
    for n, AA in zip(range(len(sample_name)), sample_name):
        ...
        # RFP蛍光強度のグラフ
        ax2 = ax.twinx()
        rfp_data = rfp_df.loc[available_samples]# / 10  # 蛍光値を10分の1にスケール
        plot_filled_line(ax2, AA, rfp_data, time, color="#FF7EF0", label="_nolegend_", ylim=(0, 25000), yticks=[5000, 10000, 25000])
...
def calculation(Growth_df, RFP_df):
    #~~~~パラメータ~~~~
    Period = 10 #測定間隔（min）
    Range = 25 #GRを算出するレンジ
    ...
    RFP = RFP_df.max(axis=1)#最大RFP蛍光強度

    NI = MGR*RFP#中性指数

    plate_visualization(MGR, RFP, NI)
    create_maindata(MGR, RFP, NI)

#-----------------------------------------------------------------------------------------------------

def plate_visualization(MGR, RFP, NI):
    #MGRの整形
    MGR = np.array(MGR.values)
    MGR = MGR.reshape([8,12])

    #RFPの整形
    RFP = np.array(RFP.values)
    RFP = RFP.reshape([8,12])
    ...
    RFP = pd.DataFrame(RFP,columns=column, index=row)
    ...
    with pd.ExcelWriter(plate_visualization_file) as writer:
        ...
        RFP.to_excel(writer,sheet_name="RFP")
        NI.to_excel(writer,sheet_name="NI(MGR_RFP)")

def create_maindata(MGR, RFP, NI):
    df = pd.DataFrame({"MGR": MGR, "RFP": RFP, "NI": NI})
    df_mean = pd.DataFrame(columns=["MGR", "RFP", "NI"])
    df_sd = pd.DataFrame(columns=["MGR", "RFP", "NI"])
    df_p = pd.DataFrame(columns=["MGR", "RFP", "NI"])
    ...
    for AA in sample_name:
        ...
        RFP_mean = RFP.loc[available_samples].mean()
        NI_mean = NI.loc[available_samples].mean()
        df_mean.loc[AA] = {"MGR": MGR_mean, "RFP": RFP_mean, "NI": NI_mean}

        MGR_sd = MGR.loc[available_samples].std()
        RFP_sd = RFP.loc[available_samples].std()
        NI_sd = NI.loc[available_samples].std()
        df_sd.loc[AA] = {"MGR": MGR_sd, "RFP": RFP_sd, "NI": NI_sd}

        t_stat, p_value_mgr = ttest_ind(MGR.loc[available_samples], MGR.loc[base_samples], equal_var=True)# moxとの比較MGR (t検定)
        t_stat, p_value_rfp = ttest_ind(RFP.loc[available_samples], RFP.loc[base_samples], equal_var=True)# moxとの比較RFP (t検定)
        t_stat, p_value_ni = ttest_ind(NI.loc[available_samples], NI.loc[base_samples], equal_var=True)# moxとの比較NI (t検定)
        df_p.loc[AA] = {"MGR" : p_value_mgr, "RFP" : p_value_rfp, "NI" : p_value_ni}

    df_mean["RFP_Φ"] = (df_mean["RFP"] - df_mean.loc["Φ", "RFP"]).apply(lambda x: max(x, 0))#Φ(自家蛍光)分を排除する
    
    # ボンフェロー二補正
    p_values_mgr = df_p["MGR"].values
    p_values_rfp = df_p["RFP"].values
    p_values_ni = df_p["NI"].values
    _, p_adj_mgr, _, _ = smm.multipletests(p_values_mgr, alpha=0.05, method='bonferroni')
    _, p_adj_rfp, _, _ = smm.multipletests(p_values_rfp, alpha=0.05, method='bonferroni')
    ...
    create_mainplot(MGR, RFP, df, df_mean, df_sd, p_adj_mgr, p_adj_rfp)
    create_NIplot(NI, df, df_mean, df_sd, p_adj_ni)

#MGR,MFI,NIを棒グラフで表す=======================================================================
def create_mainplot(MGR, RFP, df, df_mean, df_sd, p_adj_mgr, p_adj_rfp):
   ...
   # MGR 平均値と標準偏差の棒グラフ
    ...
    # RFP 平均値と標準偏差のプロット
    ax2 = ax.twinx()
    ax2.scatter(df_mean.index, df_mean['RFP_Φ'], color='#FF7EF0', marker='_', label='Expression level', s=300, edgecolor='black', linewidth=3, zorder=2, alpha=0.9)
    ax2.errorbar(df_mean.index, df_mean['RFP_Φ'], yerr=df_sd['RFP'], fmt='none', ecolor='black', elinewidth=0.5, capsize=0, zorder=1)
    ...
    for AA in sample_name:
        ...
        y_values_RFP = RFP.loc[available_samples].values#RFPの生データ
        y_values_RFP = np.maximum(y_values_RFP - df_mean.loc["Φ", "RFP"], 0)
        ax2.scatter(x_positions, y_values_RFP, color='#FF7EF0', s=10, linewidths=0.1, zorder=3, alpha=0.7)
  
    # MGRの範囲設定
    ax.set_ylim([0, 5])  # MGRの範囲で設定
    ax2.set_ylim([0,  25000])  # RFPの範囲で設定
    ...
    # 有意差のあるサンプルについて*マークをつける処理(RFP)
    for i, (p_val, rect) in enumerate(zip(p_adj_rfp, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.8, '*', ha='center', color='#FF7EF0', fontsize=10)
  
    # ラベルとタイトルの設定
    #ax.set_ylabel('Max growth rate', fontsize=12)
    #ax2.set_ylabel('Expression level', fontsize=12, rotation=270)
    ax.set_title('MGR and MFI: mCherry-PolyX (SC-LU)', fontsize=14)
...
def create_NIplot(NI, df, df_mean, df_sd, p_adj_ni):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    # ラベルとタイトルの設定
    #ax.set_ylabel('Neutral Index (MGR*RFP)', fontsize=12)
    ax.set_title('NI: mCherry-PolyX (SC-LU)', fontsize=14)
...
RFP_df = tecan_data('Label2')

#fourier_transform(Growth_df, RFP_df)
#------------------or------------------
create_lineplot(Growth_df, RFP_df)
calculation(Growth_df, RFP_df)