...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/SC-U/20210702_TDH3_poly10_U_raw.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/SC-U/sheet.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/SC-U"#保存先のパス
...
FI_YLIM = (0, 500)     # GFP曲線のylim
FI_YTICKS = [250, 500]

# --- MGR 計算パラメータ ---
PERIOD_MIN = 30           # 測定間隔（min）
RANGE = 25                # 回帰レンジ（サイクル数）
STEP_SIZE = 10            # ウインドウの移動幅（大きいほど高速）
R2_THRESHOLD = 0.9        # R^2のしきい値
GR_START_CYCLE = 0       # GR計算開始位置（サイクル数）

# --- プロット描画設定 ---
FIGSIZE = (10, 3)          # 図のサイズ(6, 3) (10, 3) 
MGR_YLIM = [0, 5]              # MGRのylim
MFI_YLIM = [0, 500]           # MFIのylim
NI_YLIM = [0, 12000]     # NIのylim
TITLE = "EGFP-PolyX (SC-U)"      # 図のタイトル
#-----------------------------
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 1800, '*', ha='center', color='k', fontsize=10)
