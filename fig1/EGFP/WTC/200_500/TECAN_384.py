...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/WTC/200_500/20250626_WTCpro_EGFP_PolyX_aTc4points_re.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/WTC/200_500/Sample_Sheet.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/EGFP/WTC/200_500"#保存先のパス

# --- サンプルの表示順（凡例・プロット順など）---
sample_name = ['Δ','A', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N','P', 'Q', 'R','S','T','V','W','Y'] #サンプル名前を指定
MDs = ["_200_", "_300_", "_400_", "_500_"]
...
GR_START_CYCLE = 15       # GR計算開始位置（サイクル数）

# --- プロット描画設定 ---
FIGSIZE = (10, 3)          # 図のサイズ
MGR_YLIM = [0, 5]              # MGRのylim
MFI_YLIM = [0, 8000]           # MFIのylim
NI_YLIM = [0, 20000]     # NIのylim
TITLE = "EGFP-PolyX (SC-LU)"      # 図のタイトル
#-----------------------------
...
Growth_df_full = tecan_data('Label1')
FI_df_full = tecan_data('Label2')
#Growth_df, FI_df = fourier_transform(Growth_df, GFP_df)
#------------------or------------------
for md in MDs :
    fit_Growth_df = pd.DataFrame()
    fit_FI_df = pd.DataFrame()

    md_clean = md.strip("_")  # "_100_" → "100"
    dir_path = os.path.join(path, md_clean)
    os.makedirs(dir_path, exist_ok=True)

    plate_visualization_file = os.path.join(dir_path, "platedata.xlsx") #最大蛍光値・MGRを保存するファイル
    lineplot_file = os.path.join(dir_path, "lineplot.pdf") #lineplotを保存するファイル
    mainplot_file = os.path.join(dir_path, "mainplot.pdf") #mainplotを保存するファイル
    NIplot_file = os.path.join(dir_path, "NIplot.pdf") #NIplotを保存するファイル
    maindata_file = os.path.join(dir_path, "maindata.xlsx") #maindataを保存するファイル

    fit_Growth_df = Growth_df_full[Growth_df_full.index.str.contains(md)]
    fit_Growth_df.index = fit_Growth_df.index.str.replace(rf"{md}", "_", regex=True)

    fit_FI_df = FI_df_full[FI_df_full.index.str.contains(md)]
    fit_FI_df.index = fit_FI_df.index.str.replace(rf"{md}", "_", regex=True)
    fit_FI_df = subtract_phi(fit_FI_df)

    create_lineplot(fit_Growth_df, fit_FI_df)
    MGR, MFI, NI, MGR_pct, MFI_pct = calculation(fit_Growth_df, fit_FI_df)
