...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/Ecoli/20250713_EColi_EGFP_PolyX.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/Ecoli/Sample_Sheet_Poly10X.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig1/Ecoli"#保存先のパス
plate_visualization_file = f'{path}/platedata.xlsx' #最大蛍光値・MGRを保存するファイル
lineplot_file = f'{path}/lineplot.pdf' #lineplotを保存するファイル
mainplot_file = f'{path}/mainplot_103.pdf' #mainplotを保存するファイル
NIplot_file = f'{path}/NIplot_103.pdf'#NIplotを保存するファイル
...
# --- ラインプロット描画設定 ---
FIN_TIME_HOUR = 40        # x軸上限 [hour]
GROWTH_YLIM = (0, 0.8)    # 増殖曲線のylim
GROWTH_YTICKS = [0.2, 0.4, 0.8]
FI_YLIM = (0, 7000)     # GFP曲線のylim
FI_YTICKS = [3500, 7000]

# --- MGR 計算パラメータ ---
PERIOD_MIN = 5           # 測定間隔（min）
RANGE = 10                # 回帰レンジ（サイクル数）
STEP_SIZE = 3            # ウインドウの移動幅（大きいほど高速）
R2_THRESHOLD = 0.9        # R^2のしきい値
GR_START_CYCLE = 3       # GR計算開始位置（サイクル数）

# --- プロット描画設定 ---
FIGSIZE = (10, 3)          # 図のサイズ
MGR_YLIM = [0, 22]              # MGRのylim
MFI_YLIM = [0, 7000]           # MFIのylim
NI_YLIM = [0, 25000]     # NIのylim
TITLE = "EGFP-PolyX (E.coli, LB + amp)"      # 図のタイトル
#-----------------------------
   ...
   # MGR 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_mgr_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 3.5, '*', ha='center', color='k', fontsize=10)
    
    # 有意差のあるサンプルについて*マークをつける処理(MFI)
    for i, (p_val, rect) in enumerate(zip(p_mfi_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 3.8, '*', ha='center', color='#58C9A7', fontsize=10)
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 3500, '*', ha='center', color='k', fontsize=10)
