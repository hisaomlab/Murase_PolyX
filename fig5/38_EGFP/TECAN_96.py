...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_EGFP/20240214_TDH3pro_EGFP_Poly10X_BY4741_38C.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_EGFP/Sample_Sheet_Poly10X.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_EGFP"#保存先のパス
plate_visualization_file = f'{path}/platedata.xlsx' #最大蛍光値・MGRを保存するファイル
lineplot_file = f'{path}/lineplot.png' #lineplotを保存するファイル
mainplot_file = f'{path}/mainplot_83.png' #mainplotを保存するファイル
NIplot_file = f'{path}/NIplot_83.png'#NIplotを保存するファイル
...
GROWTH_YTICKS = [0.4, 0.8]
FI_YLIM = (0, 200)     # GFP曲線のylim
FI_YTICKS = [100, 200]
...
GR_START_CYCLE = 70       # GR計算開始位置（サイクル数）

# --- プロット描画設定 ---
FIGSIZE = (8, 3)          # 図のサイズ
MGR_YLIM = [0, 3]              # MGRのylim
MFI_YLIM = [0, 200]           # MFIのylim
NI_YLIM = [0, 250000]     # NIのylim
TITLE = "EGFP-PolyX (SC-LU, 38˚C)"      # 図のタイトル
#-----------------------------
   ...
   # MGR 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_mgr_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.45, '*', ha='center', color='k', fontsize=10)
    
    # 有意差のあるサンプルについて*マークをつける処理(MFI)
    for i, (p_val, rect) in enumerate(zip(p_mfi_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.5, '*', ha='center', color='#58C9A7', fontsize=10)
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 40000, '*', ha='center', color='k', fontsize=10)
