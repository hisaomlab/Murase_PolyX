...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_moxGFP/20241125_mox_PolyX_38.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_moxGFP/Sample_Sheet_Poly10X.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig5/38_moxGFP"#保存先のパス
...
GROWTH_YLIM = (0, 1.0)    # 増殖曲線のylim
GROWTH_YTICKS = [0.5, 1.0]
FI_YLIM = (0, 8000)     # GFP曲線のylim
FI_YTICKS = [4000, 8000]
...
TITLE = "moxGFP-PolyX (SC-LU, 38˚C)"      # 図のタイトル
#-----------------------------
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 4000, '*', ha='center', color='k', fontsize=10)
