...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Between/20221205_GFP_poly10_RFP_LU.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Between/Sample_Sheet_Poly10X_GStoΔ.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Between"#保存先のパス
plate_visualization_file = f'{path}/platedata.xlsx' #最大蛍光値・MGRを保存するファイル
lineplot_file = f'{path}/lineplot.png' #lineplotを保存するファイル
mainplot_file = f'{path}/mainplot103.png' #mainplotを保存するファイル
NIplot_file = f'{path}/NIplot103.png'#NIplotを保存するファイル
...
# --- ラインプロット描画設定 ---
FIN_TIME_HOUR = 60        # x軸上限 [hour]
GROWTH_YLIM = (0, 1.5)    # 増殖曲線のylim
GROWTH_YTICKS = [0.5, 1.0, 1.5]
FI_YLIM = (0, 6000)     # GFP曲線のylim
FI_YTICKS = [2000, 4000, 6000]
...
MFI_YLIM = [0, 6000]           # MFIのylim
NI_YLIM = [0, 35000]     # NIのylim
TITLE = "EGFP-PolyX-GSlinker-mCherry (SC-LU)"      # 図のタイトル
#-----------------------------
...
def create_lineplot(growth_df, fi_df, rfp_df):
    ...
    for n, AA in zip(range(len(sample_name)), sample_name):
        ...
        # 赤色蛍光強度のグラフ
        rfp_data = rfp_df.loc[available_samples] / 10
        plot_filled_line(ax2, AA, rfp_data, time, color="#FF7EF0", label="_nolegend_", ylim=FI_YLIM, yticks=FI_YTICKS)
...
#============================ MGR MFI NI計算関数 =====================================
# ---- MGR MFI NI算出 ----
def calculation(Growth_df: pd.DataFrame, FI_df: pd.DataFrame, RFP_df: pd.DataFrame):
    #データをログ変換
    ...
    # ---- MFIの算出（0の場合は0.1にする） ----
    MFI_g = FI_df.max(axis=1).clip(lower=0.1)
    MFI_r = RFP_df.max(axis=1).clip(lower=0.1) / 10
    ...
    mfi_g_base = MFI_g.loc[base_samples].mean()
    mfi_r_base = MFI_r.loc[base_samples].mean()
    # 0割回避（通常は起こらないが保険）
    if mgr_base == 0: mgr_base = 1e-9
    if mfi_g_base == 0: mfi_g_base = 1e-9
    if mfi_r_base == 0: mfi_r_base = 1e-9

    MGR_pct = (MGR / mgr_base) * 100.0      # %MGR(/Δ)
    MFI_g_pct = (MFI_g / mfi_g_base) * 100.0      # %MFI_g(/Δ)
    MFI_r_pct = (MFI_r / mfi_r_base) * 100.0      # %MFI_r(/Δ)
    NI_g = MGR_pct * MFI_g_pct                  # NI_g（単位は %^2）
    NI_r = MGR_pct * MFI_r_pct                  # NI_r（単位は %^2）
    return MGR, MFI_g, MFI_r, NI_g, NI_r, MGR_pct, MFI_g_pct, MFI_r_pct

# ---- データの整形 ----
def create_maindata(MGR, MFI_g, MFI_r, NI_g, NI_r):
    # 生データをまとめる
    df = pd.DataFrame({"MGR": MGR, "MFI_g": MFI_g, "MFI_r": MFI_r, "NI_g": NI_g , "NI_r": NI_r})
    ...
    for AA in sample_name:
        ...
        if base_samples:
            ...
            p_mfi_g = ttest_ind(df.loc[available, "MFI_g"], df.loc[base_samples, "MFI_g"], equal_var=True)[1]
            p_mfi_r = ttest_ind(df.loc[available, "MFI_r"], df.loc[base_samples, "MFI_r"], equal_var=True)[1]
            p_ni_g  = ttest_ind(df.loc[available, "NI_g"],  df.loc[base_samples, "NI_g"],  equal_var=True)[1]
            p_ni_r  = ttest_ind(df.loc[available, "NI_r"],  df.loc[base_samples, "NI_r"],  equal_var=True)[1]
        else:
            p_mgr = p_mfi = p_ni = np.nan

        p_raw_dict[AA] = {"MGR": p_mgr, "MFI_g": p_mfi_g, "MFI_r": p_mfi_r, "NI_g": p_ni_g, "NI_r": p_ni_r}
    ...
    if not df_praw.empty:
        ...
        p_mfi_g_adj = smm.multipletests(df_praw["MFI_g"].fillna(1.0), alpha=0.05, method='bonferroni')[1]
        p_mfi_r_adj = smm.multipletests(df_praw["MFI_r"].fillna(1.0), alpha=0.05, method='bonferroni')[1]
        p_ni_g_adj  = smm.multipletests(df_praw["NI_g"].fillna(1.0),  alpha=0.05, method='bonferroni')[1]
        p_ni_r_adj  = smm.multipletests(df_praw["NI_r"].fillna(1.0),  alpha=0.05, method='bonferroni')[1]
        df_p = pd.DataFrame({
            "MGR_bonferroni": p_mgr_adj,
            "MFI_g_bonferroni": p_mfi_g_adj,
            "MFI_r_bonferroni": p_mfi_r_adj,
            "NI_g_bonferroni":  p_ni_g_adj,
            "NI_r_bonferroni":  p_ni_r_adj
        }, index=df_praw.index)
    else:
        df_p = pd.DataFrame(columns=["MGR_bonferroni", "MFI_g_bonferroni", "MFI_r_bonferroni", "NI_g_bonferroni", "NI_r_bonferroni"])
...
#============================ 可視化（プレート） =====================================
def plate_visualization(MGR, MFI_g, MFI_r, NI_g, NI_r):
    #MGRの整形
    MGR = np.array(MGR.values)
    MGR = MGR.reshape([8,12])

    #MFIの整形
    MFI_g = np.array(MFI_g.values)
    MFI_g = MFI_g.reshape([8,12])

    #MFIの整形
    MFI_r = np.array(MFI_r.values)
    MFI_r = MFI_r.reshape([8,12])

    #NIの整形
    NI_g = np.array(NI_g.values)
    NI_g = NI_g.reshape([8,12])

    #NIの整形
    NI_r = np.array(NI_r.values)
    NI_r = NI_r.reshape([8,12])

    column = [i for i in range(1, 13)]
    row = ["A", "B", "C", "D", "E", "F", "G", "H"]

    MGR = pd.DataFrame(MGR,columns=column, index=row)
    MFI_g = pd.DataFrame(MFI_g,columns=column, index=row)
    MFI_r = pd.DataFrame(MFI_r,columns=column, index=row)
    NI_g = pd.DataFrame(NI_g,columns=column, index=row)
    NI_r = pd.DataFrame(NI_r,columns=column, index=row)

    with pd.ExcelWriter(plate_visualization_file) as writer:
        MGR.to_excel(writer,sheet_name="MGR")
        MFI_g.to_excel(writer,sheet_name="MFI_g")
        MFI_r.to_excel(writer,sheet_name="MFI_r")
        NI_g.to_excel(writer,sheet_name="NI_g")
        NI_r.to_excel(writer,sheet_name="NI_r")
...
def create_mainplot(results):
    ...
    p_mfi_g_adj = df_p.get("MFI_g_bonferroni", pd.Series(1.0, index=df_mean.index)).reindex(df_mean.index)
    p_mfi_r_adj = df_p.get("MFI_r_bonferroni", pd.Series(1.0, index=df_mean.index)).reindex(df_mean.index)
   ...
   # MGR 平均値と標準偏差の棒グラフ
    ...
    # MFI 平均値と標準偏差のプロット
    ax2 = ax.twinx()
    ax2.scatter(df_mean.index, df_mean['MFI_g'], color='#58C9A7', marker='_', label='Expression level', s=150, edgecolor='black', linewidth=2.5, zorder=2, alpha=0.9)
    ax2.errorbar(df_mean.index, df_mean['MFI_g'], yerr=df_sd['MFI_g'], fmt='none', ecolor='black', elinewidth=0.5, capsize=0, zorder=1)
    # RFPも同様に
    ax2.scatter(df_mean.index, df_mean['MFI_r'], color='#FF7EF0', marker='_', label='Expression level', s=150, edgecolor='black', linewidth=2.5, zorder=2, alpha=0.9)
    ax2.errorbar(df_mean.index, df_mean['MFI_r'], yerr=df_sd['MFI_r'], fmt='none', ecolor='black', elinewidth=0.5, capsize=0, zorder=1)
    ...
    for AA in sample_name:
        ...
        y_values_MFI_g = df.loc[available, "MFI_g"].values#MFI_gの生データ
        ax2.scatter(x_positions, y_values_MFI_g, color='#58C9A7', s=10, linewidths=0.1, zorder=3, alpha=0.7)

        y_values_MFI_r = df.loc[available, "MFI_r"].values#MFI_rの生データ
        ax2.scatter(x_positions, y_values_MFI_r, color='#FF7EF0', s=10, linewidths=0.1, zorder=3, alpha=0.7)
    ...
    # 有意差のあるサンプルについて*マークをつける処理(MFI_g)
    for i, (p_val, rect) in enumerate(zip(p_mfi_g_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.8, '*', ha='center', color='#58C9A7', fontsize=10)

    # 有意差のあるサンプルについて*マークをつける処理(MFI_r)
    for i, (p_val, rect) in enumerate(zip(p_mfi_r_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 0.85, '*', ha='center', color='#FF7EF0', fontsize=10)
...
def create_NIplot(results):
    ...
    df_sd   = results["SD"]
    df_p    = results["P-Values"]
    p_ni_g_adj = df_p.get("NI_g_bonferroni", pd.Series(1.0, index=df_mean.index)).reindex(df_mean.index)
    p_ni_r_adj = df_p.get("NI_r_bonferroni", pd.Series(1.0, index=df_mean.index)).reindex(df_mean.index)

    fig, ax = plt.subplots(figsize=FIGSIZE)# プロットの準備

    # x位置（カテゴリ）とバー幅
    labels = df_mean.index.tolist()
    x = np.arange(len(labels))
    width = 0.4

   # NI 平均値と標準偏差の棒グラフ
    bars_g = ax.bar(x - width/2, df_mean['NI_g'], color="#D0B9A4", width=0.3, linewidth=0.5, edgecolor='#58C9A7', alpha=0.7)
    ax.errorbar(x - width/2, df_mean['NI_g'], yerr=df_sd['NI_g'], fmt='none', ecolor='black', elinewidth=0.5, capsize=0)

    bars_r =ax.bar(x + width/2, df_mean['NI_r'], color="#D0B9A4", width=0.3, linewidth=0.5, edgecolor='#FF7EF0', alpha=0.7)
    ax.errorbar(x + width/2, df_mean['NI_r'], yerr=df_sd['NI_r'], fmt='none', ecolor='black', elinewidth=0.5, capsize=0)

    # 生データのドットプロット
    for i, AA in enumerate(labels):
        available = [s for s in [AA + "_1", AA + "_2", AA + "_3", AA + "_4"] if s in df.index]
        
        y_values_NI_g = df.loc[available, "NI_g"].values#NIの生データ
        x_positions_g = np.full(len(y_values_NI_g), x[i] - width/2, dtype=float)
        ax.scatter(x_positions_g, y_values_NI_g, color='black', s=5, edgecolor='black', linewidths=0.1, zorder=3, alpha=0.5)

        y_values_NI_r = df.loc[available, "NI_r"].values#NIの生データ
        x_positions_r = np.full(len(y_values_NI_r), x[i] + width/2, dtype=float)
        ax.scatter(x_positions_r, y_values_NI_r, color='black', s=5, edgecolor='black', linewidths=0.1, zorder=3, alpha=0.5)
  
    # NIの範囲設定
    ax.set_ylim(NI_YLIM)

    # 有意差のあるサンプルについて*マークをつける処理
    for i, (p_val, rect) in enumerate(zip(p_ni_g_adj, bars_g)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 5500, '*', ha='center', color='k', fontsize=10)

    # 有意差のあるサンプルについて*マークをつける処理
    for i, (p_val, rect) in enumerate(zip(p_ni_r_adj, bars_r)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 5500, '*', ha='center', color='k', fontsize=10)
...
RFP_df = tecan_data('Label3')

FI_df = subtract_phi(GFP_df)
RFP_df = subtract_phi(RFP_df)

#Growth_df, FI_df = fourier_transform(Growth_df, FI_df)
#------------------or------------------
create_lineplot(Growth_df, FI_df, RFP_df)
MGR, MFI_g, MFI_r, NI_g, NI_r, MGR_pct, MFI_g_pct, MFI_r_pct = calculation(Growth_df, FI_df, RFP_df)
plate_visualization(MGR, MFI_g, MFI_r, NI_g, NI_r)
results = create_maindata(MGR, MFI_g, MFI_r, NI_g, NI_r)
