...
# ========================= ユーザー設定（入出力・描画） =========================
# --- 入力ファイル ---
TECAN_file_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Inside/20250619_EGFP173_PolyX_174EGFP.xlsx" #TECANのraw_dataを指定
sample_sheet_name = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Inside/Sample_Sheet_Poly10X.xlsx" #サンプルシートを指定

# --- 出力先 ---
path = "/Users/muraseyukihiro/Desktop/PolyX_paper/new/fig3/Inside"#保存先のパス
...
NI_YLIM = [0, 12000]     # NIのylim
TITLE = "EGFP-insidePolyX (SC-LU)"      # 図のタイトル
#-----------------------------
...
def create_NIplot(results):
   ...
   # NI 平均値と標準偏差の棒グラフ
    ...
    for i, (p_val, rect) in enumerate(zip(p_ni_adj, bars)):
        if p_val < 0.05:  # 有意差のある場合
            ax.text(rect.get_x() + rect.get_width() / 2., rect.get_y() - 2000, '*', ha='center', color='k', fontsize=10)
